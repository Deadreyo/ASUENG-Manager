...
export default function FolderToObject(dir: string): ProjectObject {
  const totalObj = FolderToObjectIterator(dir);
...
function FolderToObjectIterator(dir: string): ProjectObject[] {
  ...
  readdirSync(dir).forEach((fileName) => {
    ...
    if (isDir) {
      children = FolderToObjectIterator(filepath);
    }
    ...
    if (extname(fileName) === '.txt') {
      if (fileName.slice(0, 4) === 'note')
        obj.note = [
          { message: readFileSync(filepath).toString(), date: obj.date },
        ];
      if (fileName.slice(0, 7) === 'credits')
        obj.credits = [...readFileSync(filepath).toString().split('\n')];
    } else {
      if (isDir) {
        if (children && Array.isArray(children))
          notesCreditsTransfer(obj, children);
...
function notesCreditsTransfer(obj: ProjectObject, children: ProjectObject[]) {
  children
    .filter((child) => child.note !== undefined)
    .forEach((child) => {
  ...
  children
    .filter((child) => child.credits !== undefined)
    .forEach((child) => {
