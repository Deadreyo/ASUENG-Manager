...
export default function FolderToObject(
  dir: string,
  mode: 'normal' | 'link' = 'normal'
): ProjectObject {
...
function FolderToObjectIterator(
  dir: string,
  mode: 'normal' | 'link' = 'normal'
): ProjectObject[] {
  ...
  children
    ...
    .forEach((child) => {
      ...
      if (child.note && !child.children) {
        ...
        (children as ProjectObject[]).splice(
          (children as ProjectObject[]).indexOf(child),
          1
        );
      }
    });
